    ...
    async def start(self):
       ...
       if FORCE_SUB_GRUP:
            try:
                ...
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot can't Export Invite link from Force Sub Channel!")
                self.LOGGER(__name__).warning(f"Please Double check the FORCE_SUB_GRUP value and Make sure Bot is Admin in channel with Invite Users via Link Permission, Current Force Sub Channel Value: {FORCE_SUB_GRUP}")
                self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/gkushskap for support")
                sys.exit()
        try:
