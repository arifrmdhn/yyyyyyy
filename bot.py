...
from config import API_HASH, APP_ID, LOGGER, TG_BOT_TOKEN, TG_BOT_WORKERS, FORCE_SUB_CHANNEL1, FORCE_SUB_CHANNEL2, FORCE_SUB_GRUP, CHANNEL_ID
    ...
    async def start(self):
        ...
        if FORCE_SUB_GRUP:
            try:
                ...
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot can't Export Invite link from Force Sub Grup!")
                self.LOGGER(__name__).warning(f"Please Double check the FORCE_SUB_GRUP value and Make sure Bot is Admin in grup with Invite Users via Link Permission, Current Force Sub Channel Value: {FORCE_SUB_GRUP}")
                self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/gkushskap for support")
                sys.exit()
        try:
